...
public class DBUserStorageProvider implements UserStorageProvider,
    ...
    public Stream getUserByAttribute(RealmModel realm, String attribute, String value){
        //return Stream.empty();
        log.infov("lookup user by attr: attr={0} value={1}", attribute, value);
        Map<String, String> user = repository.findByWallet(value);
        log.infov("lookup user from repo {0}", user);

        if (user == null) {
            log.debugv("etUserByAttribute returned null, skipping creation of UserAdapter, expect login error");
            return null;
        } else {
            UserModel userFromWallet =  new UserAdapter(session, realm, model, user, allowDatabaseToOverwriteKeycloak);
            log.infov("lookup user {0}", userFromWallet);
            return Stream.of(userFromWallet);
        }
    }
    ...
    {
        //return Stream.empty();
        log.infov("search for group members: realm={0} attrName={1} attrValue={2}", realm.getId(), "address", attrValue);
        return getUserByAttribute(realm, "address", attrValue.toUpperCase());
    }
